...
class RatipathBot {
    ...
    isAvatarMessage(message) {
        return this.avatars.some(avatar => message.author.username === `${avatar.name} ${avatar.emoji || ''}`.trim());
    }
    ...
    async processMessages(channel, avatar) {
        ...
        if (result.trim() !== "") {
            ...
            this.addToConversationHistory(avatar.name, { role: "assistant", content: result });
    ...
    shouldContinueConversation(avatarName) {
        const recentHistory = this.conversationHistories[avatarName].slice(-this.interactionLimit);
    ...
    async initiateAvatarInteraction(avatar, channel) {
        ...
        const otherAvatar = otherAvatars[Math.floor(Math.random() * otherAvatars.length)];

        const result = await this.chatWithAI(otherAvatar);

        if (result.trim() !== "") {
            console.log(`🧠 ${otherAvatar.name} responds to ${avatar.name}:`, result);
        ...
        try {
            const response = await this.openai.chat.completions.create({
                ...
                messages: [
                    ...
                    {
                        role: 'user',
                        content: 'respond naturally to the above conversation, in character, driving the narrative forward and pursuing your goals.'
                    }
                ],
            });
    ...
    async handleMessage(message) {
        ...
        const targetAvatar = mentionedAvatar || this.getHomeChannelAvatar(message.channel.id);
        ...
        if (targetAvatar) {
            ...
            const imageUrls = message.attachments.filter(att => att.contentType.startsWith('image/')).map(att => ({
                type: "image_url",
                image_url: {
                    url: att.url,
                    detail: "auto"
            ...
            if (content !== '') {
                messageContent.push({ type: "text", text: `(${message.channel.name}) ${message.author.username}: ${content}` });
            }
    ...
    async handleChannelDecay(avatar) {
        ...
        if (lastMention && !isHomeChannel && Date.now() - lastMention.timestamp > this.channelDecayTime) {
            delete this.lastMentionedChannels[avatar.name];
            console.log(`🕒 ${avatar.name} has decayed back to their home channel.`);
        }
    }
    ...
    async connectToMongoDB() {
        try {
            ...
            this.db = this.mongoClient.db('ratipath_db');
            this.avatarsCollection = this.db.collection('avatars');
            this.memoryCollection = this.db.collection('memories');

            const avatarsFromJson = JSON.parse(fs.readFileSync('characters.json', 'utf-8'));

            for (const avatar of avatarsFromJson) {
                const existingAvatar = await this.avatarsCollection.findOne({ name: avatar.name });
                if (!existingAvatar) {
                    await this.avatarsCollection.insertOne(avatar);
                } else {
                    await this.avatarsCollection.updateOne({ name: avatar.name }, { $set: avatar });
                }
            }
    ...
    async loadAvatars() {
        try {
            ...
            console.log('🧠 Avatars loaded:', this.avatars.map(avatar => avatar.name));
        } catch (error) {
